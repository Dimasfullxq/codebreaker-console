...
RSpec.describe Console do
  ...
  describe '#start' do
    it 'loses the game' do
      ...
      expect(console).to receive(:finish).with(GameEnding::START_NEW_GAME_MESSAGE, game)
      console.start(game)
    end
    ...
    it 'wins the game' do
      ...
      expect(console).to receive(:gets).and_return('1234', 'n')
      console.start(game)
    end
    ...
    it 'takes a hint if you have at least one left' do
      ...
      allow(console).to receive(:finish).with(GameEnding::START_NEW_GAME_MESSAGE, game)
      expect(console).to receive(:gets).and_return('hint', '1234')
      console.start(game)
    end
